...
function restartBaby(): number {
  const started = Date.now();
  const result = spawnSync('/usr/bin/systemctl', ['restart', 'baby-quirt.service'], {
    encoding: 'utf8', timeout: 45_000,
  });
  if (result.status !== 0) throw new Error(`baby-quirt.service restart failed: ${result.stderr.trim()}`);
  const active = spawnSync('/usr/bin/systemctl', ['is-active', '--quiet', 'baby-quirt.service'], {
    timeout: 10_000,
  });
  if (active.status !== 0) throw new Error('baby-quirt.service is not active after restart');
...
async function httpReadback(): Promise<Record<string, unknown>> {
  ...
  const authorization = await fetch(
    'https://baby-quirt.stealtheye.io/.well-known/oauth-authorization-server',
    { signal: AbortSignal.timeout(10_000) },
  );
  const resource = await fetch(
    'https://baby-quirt.stealtheye.io/.well-known/oauth-protected-resource',
    { signal: AbortSignal.timeout(10_000) },
  );
  const mcp = await fetch('https://baby-quirt.stealtheye.io/mcp', {
...
if (recordPath === undefined || !recordPath.startsWith('/var/lib/baby-quirt/skill-deployments/')) {
