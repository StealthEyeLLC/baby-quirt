...
export const SKILL_DEPLOYMENT_ROOT = '/var/lib/baby-quirt/skill-deployments';
export const SKILL_STAGING_ROOT = '/var/lib/baby-quirt/skill-staging';
...
export class SkillDeploymentService {
  ...
  static handles(operation: string): boolean {
    return ['baby.skill.deploy', 'baby.skill.status', 'baby.skill.rollback', 'baby.skill.list'].includes(operation);
  }
  ...
  private async deploy(requestId: string, body: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      const retainedDefinitions = this.loaded().definitions.filter((definition) => !definition.operation.startsWith(`baby.skill.${manifest.name}.`));
  ...
  private status(): Record<string, unknown> {
    ...
    return {
      ...
      currentHealth: { status: 'healthy', product: 'baby-quirt' },
