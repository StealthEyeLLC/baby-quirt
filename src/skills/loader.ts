...
export const DEFAULT_SKILL_ROOT = '/opt/baby-quirt/skills';
...
const OPERATION = /^baby\.skill\.[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?\.[a-z0-9._-]{1,128}$/;
const SAFE_REPOSITORY = /^StealthEyeLLC\/[A-Za-z0-9_.-]{1,100}$/;
const SAFE_GIT = /^[a-f0-9]{40}$/;
const MANIFEST_KEYS = ['schemaVersion', 'name', 'version', 'entrypoint', 'babyCompatibility', 'operations'] as const;
...
export function validateSkillManifest(value: unknown): SkillManifest {
  ...
  if (typeof value.babyCompatibility !== 'string' || !['0.1.0', '>=0.1.0 <1.0.0'].includes(value.babyCompatibility)) {
    throw new SkillValidationError('babyCompatibility is unsupported');
  }
  ...
  const operations = value.operations.map((candidate, index): SkillOperationManifest => {
    ...
    if (!candidate.operation.startsWith(`baby.skill.${value.name}.`)) {
  ...
  return {
    ...
    entrypoint,
    babyCompatibility: value.babyCompatibility,
    operations,
  };
}
