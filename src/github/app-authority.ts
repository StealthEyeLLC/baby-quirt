...
const CREDENTIAL_NAME = 'github-baby-quirt-app-private-key';
...
export class GitHubAppAuthority {
  ...
  async verify(body: Record<string, unknown>): Promise<Record<string, unknown>> {
    const repository = safeRepository(body.repository ?? 'StealthEyeLLC/baby-x');
  ...
  async proof(body: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const branch = `baby-quirt-authority-proof-${suffix}`;
    const path = `.baby-quirt/github-app-authority-proof-${suffix}.json`;
    ...
    try {
      ...
      const commit = await this.request<CommitView>(`/repos/${repository}/git/commits`, {
        ...
        authorization: token.value,
        body: {
          message: 'test: prove Baby Quirt GitHub App authority',
          tree: treeSha,
  ...
  private async request<T = Record<string, never>>(
    ...
    const response = await this.fetchFn(`${API_ROOT}${path}`, {
      ...
      headers: {
        ...
        'User-Agent': 'baby-quirt-github-authority',
