...
async function inFixture(run: (fixture: Fixture) => void | Promise<void>): Promise<void> {
  const root = mkdtempSync(join(tmpdir(), 'baby-skill-loader-test-'));
...
function operation(
  operationName = 'baby.skill.proof.echo',
  handler = 'echo',
...
function sourceEntry(bundle: ReturnType<typeof installBundle>): PackageSetSkill {
  return {
    ...
    sourceRepository: 'StealthEyeLLC/baby-quirt',
    sourceCommit: COMMIT,
    sourceTree: TREE,
...
function materialization(value = manifest(), moduleText?: string): GitHubSkillMaterialization {
  ...
  return {
    repository: 'StealthEyeLLC/baby-quirt', ref: COMMIT, commit: COMMIT, tree: TREE,
...
describe('minimal trusted skill loader', () => {
  ...
  it('10. wrong repository owner is rejected before GitHub access', async () => {
    await inFixture(async (fixture) => {
      ...
      await assert.rejects(
        () => service.execute('baby.skill.deploy', 'request', {
          repository: 'Other/baby-quirt', ref: COMMIT, skillPath: 'examples/skills/proof-echo',
        }),
        /StealthEyeLLC/,
      );
  ...
  it('11. exact commit mismatch is rejected', async () => {
    await inFixture(async (fixture) => {
      ...
      const fetchFn: typeof fetch = async (input, init = {}) => {
        ...
        if (url.pathname === '/app') return githubResponse(200, { id: 4380878, name: 'Baby Quirt GitHub Authority', slug: 'baby-quirt-github-authority', owner: { login: 'StealthEyeLLC' } });
        ...
        if (url.pathname.endsWith('/access_tokens')) return githubResponse(201, { token: 'installation-token-value-long-enough', repositories: [{ full_name: 'StealthEyeLLC/baby-quirt' }] });
      ...
      await assert.rejects(
        () => authority.materializeSkill('StealthEyeLLC/baby-quirt', 'main', 'examples/skills/proof-echo', 'c'.repeat(40)),
        /expectedCommit/,
      );
    });
  });
  ...
  it('16. operation namespace violations are rejected', () => {
    assert.throws(
      () => validateSkillManifest(manifest({ operations: [operation('baby.skill.other.echo')] })),
  ...
  it('25. smoke failure triggers automatic rollback', async () => {
    await inFixture(async (fixture) => {
      ...
      replacePointer(fixture.currentLink, prior.path);
      const result = await activateRecord(writeRecord(fixture, activationRecord(prior, candidate, {
        expectedSkillOperations: ['baby.skill.proof.echo'],
        smokeOperation: 'baby.skill.proof.echo', smokePayload: { text: 'proof' },
      })), {
        currentLink: fixture.currentLink, previousLink: fixture.previousLink,
        releaseRoot: fixture.releaseRoot, coreDefinitions: CORE,
        restart: () => 1, healthReadback: async () => ({ durationMs: 1 }),
  ...
  it('28. identical redeployment reuses the exact bundle directory', async () => {
    await inFixture(async (fixture) => {
      ...
      const first = await service.execute('baby.skill.deploy', 'request-one', request) as Record<string, unknown>;
      const second = await service.execute('baby.skill.deploy', 'request-two', request) as Record<string, unknown>;
  ...
  it('29. restart loading uses the active package-set pointer', async () => {
    await inFixture(async (fixture) => {
      ...
      const loaded = await loadActivePackageSet(CORE, fixture.currentLink, fixture.releaseRoot);
      assert.equal(loaded.setDigest, set.digest);
      assert.deepEqual(loaded.definitions.map((item) => item.operation), ['baby.skill.proof.echo']);
    });
  });

  it('30. baby.describe reports the active set and loaded skills', async () => {
    await inFixture(async (fixture) => {
      const bundle = installBundle(fixture);
      const set = writeSet(fixture, [sourceEntry(bundle)], definitionsFor(bundle.manifest), '2026-07-27T00:00:00.000Z');
      const loaded = await loadPackageSetFromPath(set.path, CORE, fixture.releaseRoot);
      ...
      assert.equal(description.operations.some((item) => item.operation === 'baby.skill.proof.echo'), true);
    });
  });

  it('31. baby.skill.status returns accurate active identities and counts', async () => {
    await inFixture(async (fixture) => {
      const context = await createService(fixture);
      const deployment = await context.service.execute('baby.skill.deploy', 'status-deploy', {
        repository: 'StealthEyeLLC/baby-quirt', ref: COMMIT,
        skillPath: 'examples/skills/proof-echo', expectedCommit: COMMIT,
      ...
      const status = await context.service.execute('baby.skill.status', 'status', {}) as {
  ...
  it('32. baby.skill.list output is bounded', async () => {
    await inFixture(async (fixture) => {
      const context = await createService(fixture);
      for (let index = 0; index < 205; index += 1) {
        mkdirSync(join(fixture.releaseRoot, index.toString(16).padStart(64, '0')));
      }
      const result = await context.service.execute('baby.skill.list', 'list', {}) as {
  ...
  it('33. installation tokens are not persisted', async () => {
    await inFixture(async (fixture) => {
      const context = await createService(fixture);
      await context.service.execute('baby.skill.deploy', 'token-scan', {
        repository: 'StealthEyeLLC/baby-quirt', ref: COMMIT,
        skillPath: 'examples/skills/proof-echo', expectedCommit: COMMIT,
      });
