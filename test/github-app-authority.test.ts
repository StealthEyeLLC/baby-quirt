...
const REPOSITORY = 'StealthEyeLLC/baby-x';
...
function fixture(): { directory: string; keyPath: string } {
  const directory = mkdtempSync(join(tmpdir(), 'baby-github-app-test-'));
...
function mockGitHub(): { fetchFn: typeof fetch; calls: Array<{ method: string; path: string; body?: unknown }>; deleted: () => boolean } {
  ...
  const fetchFn: typeof fetch = async (input, init = {}) => {
    ...
    if (method === 'GET' && url.pathname === '/app') {
      ...
      return response(200, { id: Number(APP_ID), name: 'Baby GitHub Authority', slug: 'baby-github-authority', owner: { login: ACCOUNT } });
    }
...
describe('GitHubAppAuthority', () => {
  it('verifies exact App and installation identity with a repository-scoped read token', async () => {
    ...
    try {
      ...
      assert.deepEqual(result.app, { id: APP_ID, name: 'Baby GitHub Authority', slug: 'baby-github-authority', owner: ACCOUNT });
